          ...
          <div className="flex min-h-full items-center justify-center">
            <form onSubmit={handleSubmit} className="w-full max-w-[520px] rounded-xl bg-canvas p-6">
              ...
              <div className="mt-4 space-y-4">
                <div><label className="block text-sm font-medium text-body">Judul</label><input value={form.title ?? ""} onChange={(event) => setForm({ ...form, title: event.target.value })} required className="mt-1 h-[38px] w-full rounded-md border border-hairline bg-canvas px-3 text-sm text-ink outline-none focus:border-primary" /></div>
                <div><label className="block text-sm font-medium text-body">Jenis</label><select value={form.event_type ?? "kegiatan"} onChange={(event) => setForm({ ...form, event_type: event.target.value })} className="mt-1 h-[38px] w-full rounded-md border border-hairline bg-canvas px-3 text-sm text-ink capitalize">{eventTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></div>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div><label className="block text-sm font-medium text-body">Tanggal Mulai</label><input type="date" value={isoDate(form.start_date)} onChange={(event) => setForm({ ...form, start_date: event.target.value })} required className="mt-1 h-[38px] w-full rounded-md border border-hairline bg-canvas px-2 text-sm text-ink" /></div>
                  <div><label className="block text-sm font-medium text-body">Tanggal Selesai</label><input type="date" value={isoDate(form.end_date)} onChange={(event) => setForm({ ...form, end_date: event.target.value })} required className="mt-1 h-[38px] w-full rounded-md border border-hairline bg-canvas px-2 text-sm text-ink" /></div>
                </div>
                <div><label className="block text-sm font-medium text-body">Deskripsi</label><textarea value={form.description ?? ""} onChange={(event) => setForm({ ...form, description: event.target.value })} className="mt-1 w-full rounded-md border border-hairline bg-canvas px-3 py-2 text-sm text-ink outline-none focus:border-primary" rows={3} /></div>
